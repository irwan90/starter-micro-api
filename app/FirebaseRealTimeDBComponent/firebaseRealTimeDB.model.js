...
exports.getAllProject = function(result) {
    let ref = firebaseSettings.database().ref('Project');
    ref.once('value', function(snapshot) {
        result(null, snapshot.val());
    });
};

exports.getAllProjectByGroup = function(groupId, result) {
    let ref = firebaseSettings.database().ref('Project/' + groupId);
    ref.once('value', function(snapshot) {
        result(null, snapshot.val());
    });

    // ref.orderByChild("Status_Close").on("child_added", snap => {
    //     result(null, snap.val());
    //     console.log(snap.val());
    // });
    // firebaseSettings.database().ref('Project/' + groupId).once('value').then(function(snapshot) {
    //     result(null, JSON.stringify(snapshot.val()));
    // });
};